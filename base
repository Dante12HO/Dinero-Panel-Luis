...
<body>
  <h1>💰 Dinero de jugadores (actualizado cada 24h)</h1>
  ...
  <button id="addMoneyBtn">➕ Agregar dinero</button>
  <button id="subtractMoneyBtn">➖ Restar dinero</button>
  <br><br>
  ...
  <button id="updateDailyBtn">💼 Actualizar ingreso/gasto diario</button>

  <script>
    const initialPlayers = [
      { name: "Jon", country: "Alemania", money: 2.0, dailyIncome: 4.6, dailyExpense: 2.3 },
      { name: "Noah", country: "España", money: 5.0, dailyIncome: 1.6, dailyExpense: 0 },
      ...
      { name: "Extra", country: "País X", money: 0, dailyIncome: 1.0, dailyExpense: 0 }
    ];
    ...
    function render() {
      ...
      players.forEach(p => {
        ...
        row.innerHTML = `<td>${p.name} (${p.country})</td><td>€${p.money.toFixed(2)}B</td><td>€${p.dailyIncome.toFixed(2)}B</td><td>€${p.dailyExpense.toFixed(2)}B</td>`;
    ...
    function updateClock() {
      ...
      document.getElementById('clock').textContent = '⌚ Hora en Barcelona: ' + now;
